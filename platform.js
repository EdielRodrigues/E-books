(() => {
  ...
  let auth, db, currentUser, currentProfile = {}, currentPayment = null, selectedPlanId = null, selectedPlan = null, presenceTimer = null, presenceRef = null, pixCheckTimer = null, pixChecking = false, mp = null, cardBrickController = null;
  ...
  function selectPlan(planId){selectedPlanId=planId;selectedPlan=cfg.plans?.[planId];if(!selectedPlan)return toast('Plano inválido.',true);$('#plansArea').classList.add('hidden');$('#paymentMethodPanel').classList.remove('hidden');$('#pixPanel').classList.add('hidden');$('#cardPanel').classList.add('hidden');$('#tryPixAfterCard')?.classList.add('hidden')}
  function backToPlans(){stopPixAutoCheck();currentPayment=null;selectedPlanId=null;selectedPlan=null;cardBrickController?.unmount?.();cardBrickController=null;$('#paymentMethodPanel').classList.add('hidden');$('#pixPanel').classList.add('hidden');$('#cardPanel').classList.add('hidden');$('#plansArea').classList.remove('hidden')}
  ...
  async function loadLatestPayment(){if(!currentUser||!cfg.functionsBaseUrl)return;try{const out=await api('/latestPayment');if(out.payment&&out.payment.status!=='approved'){currentPayment=out.payment;if(out.payment.paymentMethod==='card'){selectedPlanId=out.payment.planId;selectedPlan=cfg.plans?.[selectedPlanId];$('#plansArea').classList.add('hidden');$('#paymentMethodPanel').classList.add('hidden');$('#cardPanel').classList.remove('hidden');$('#cardPlanName').textContent=out.payment.planName||'Plano';$('#cardAmount').textContent=money(out.payment.amount);setStatusCard(out.payment.status,out.payment.statusDetail)}else{showPix(currentPayment)}startPixAutoCheck()}}catch(e){console.warn(e.message)}}
  ...
  async function logout(){currentPayment=null;stopPixAutoCheck();stopPresence();await auth.signOut()}
